...
function CarregaDll()
{
    if (strpos(PHP_OS, 'WIN') === false) {
        $prefixo = strtolower("libacbrconsultacnpj");
        $extensao = ".so";
    } else {
    ...
    if (file_exists($dllPath))
    {
        ...
        echo json_encode(["mensagem" => "Biblioteca (.dll/.so) não encontrada no caminho especificado: " . $dllPath]);
        return -10;
    }
...
{
    $importsPath = __DIR__ . DIRECTORY_SEPARATOR . 'ACBrConsultaCNPJST.h';
